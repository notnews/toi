...
sns.set_theme(context="notebook", font_scale=1.05, style="whitegrid")
BLUE = "#0077BB"
PURPLE = "#AA3377"
...
def coverage_dayweek(
    ...
    data: List[float],
    figsize: Tuple = (12, 5),
    cmap: Union[str, LinearSegmentedColormap, ListedColormap] = sns.color_palette(
        "mako_r", as_cmap=True
    ),
    ...
    **kwargs,
    ...
    july.heatmap(dates, data, title=title, cmap=cmap, ax=ax)
...
def coverage_mthyr(
    ...
    **heatmap_kwargs,
    ...
    sns.heatmap(
        ...
        **heatmap_kwargs,
        cbar_kws={
            "shrink": cbar_shrink,
            "pad": cbar_pad,
            "format": FuncFormatter(lambda x, p: format(int(x), ",")),
        },
        ax=ax,
    )
    ...
    if xtickangle:
        plt.xticks(rotation=xtickangle, fontsize=9, fontweight="normal", ha="center")
...
def plotline(
    ...
    title=None,
    yrange=None,
    xrange=range(0, 253, 36),
    xticklabels=None,
    color=BLUE,
    figsize=(12 * 0.7, 8 * 0.7),
    tickersize=13,
    markersize=80,
    labsize=15,
    titlesize=18,
    ...
    sns.lineplot(x=x, y=ynew_line, linewidth=4, alpha=0.85, color=PURPLE, ax=ax)
    ...
    ax.set_ylim(ax.set_ylim()[0], ax.set_ylim()[1] + 0.5)
...
def dow_plot(y, x, data):
    ...
    fig, ax = plt.subplots(figsize=(12 * 0.5, 8 * 0.5))

    sns.pointplot(
        x=x, y=y, data=data, color="black", order=dow, ci=95, errwidth=2, join=False, ax=ax,
    )
    sns.pointplot(
        x=x,
        y=y,
